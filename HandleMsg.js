...
module.exports = HandleMsg = async (aruga, message) => {
	...
	case 'cooltext':
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                await alexander.reply(from, `Tidak ada gambar! Untuk menggunakan ${prefix}sticker\n\n\nKirim gambar dengan caption\n${prefix}sticker <biasa>\n${prefix}sticker nobg <tanpa background>\n\natau Kirim pesan dengan\n${prefix}sticker <link_gambar>`, id)
            }
            break
            case 'antisticker':
            case 'antistiker':
                    if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            ...
                            alexander.reply(from, '*[Anti Sticker SPAM]* telah di aktifkan\nSetiap member grup yang spam sticker lebih dari 7 akan di kick oleh bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return alexander.reply(from, '*Anti Spam Sticker Detector* sudah non-aktif di grup ini', id) //if number already exists on database
                        } else {
                            let nixx = antisticker.indexOf(chatId)
                            antisticker.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            alexander.reply(from, '*[Anti Sticker SPAM]* telah di nonaktifkan\n', id)
                        }
                    } else {
                        alexander.reply(from, `pilih on / off\n\n*[Anti Sticker SPAM]*\nSetiap member grup yang spam sticker akan di kick oleh bot!`, id)
                    }
                    break
                    case 'antilink':
                    if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Wahai admin, jadikan saya sebagai admin grup dahulu :)', id)
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return alexander.reply(from, '*Anti Group Link Detector* sudah aktif di grup ini', id) //if number already exists on database
                        } else {
                            antilink.push(chatId)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            alexander.reply(from, '*[Anti Group Link]* telah di aktifkan\nSetiap member grup yang mengirim pesan mengandung link grup akan di kick oleh bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
                            return alexander.reply(from, '*Anti Group Link Detector* sudah non-aktif di grup ini', id) //if number already exists on database
                        } else {
                            let nixx = antilink.indexOf(chatId)
                            antilink.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            alexander.reply(from, '*[Anti Group Link]* telah di nonaktifkan\n', id)
                        }
                    } else {
                        alexander.reply(from, `pilih on / off\n\n*[Anti Group Link]*\nSetiap member grup yang mengirim pesan mengandung link grup akan di kick oleh bot!`, id)
                    }
                    break  
	case 'brainly':
            if (!isGroupMsg) return alexander.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            ...
            if (args.length >= 2){
                ...
                alexander.reply(from, `➸ *Pertanyaan* : ${tanya.split('.')[0]}\n\n➸ *Jumlah jawaban* : ${Number(jum)}`, id)
                await BrainlySearch(tanya.split('.')[0],Number(jum), function(res){
                    res.forEach(x=>{
                        if (x.jawaban.fotoJawaban.length == 0) {
                            alexander.reply(from, `➸ *Pertanyaan* : ${x.pertanyaan}\n\n➸ *Jawaban* : ${x.jawaban.judulJawaban}\n`, id)
			    alexander.sendText(from, 'Selesai ✅, donasi kesini ya paypal.me/TheSploit | Pulsa : 085754337101')
                        } else {
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    alexander.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 min!', id)
                    var filename = `./media/stickergif.${mimetype.split('/')[1]}`
                    await fs.writeFileSync(filename, mediaData)
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        var gif = await fs.readFileSync('./media/stickergf.gif', { encoding: "base64" })
                        await alexander.sendImageAsSticker(from, `data:image/gif;base64,${gif.toString('base64')}`)
                        .catch(() => {
                            aruga.reply(from, 'Maaf filenya terlalu besar!', id)
                        })
                    })
                  } else {
                    alexander.reply(from, `[❗] Kirim gif dengan caption *${prefix}stickergif* max 10 sec!`, id)
                   }
                } else {
		    alexander.reply(from, `[❗] Kirim gif dengan caption *${prefix}stickergif*`, id)
	        }
            break
        case 'stikergiphy':
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                if (!getGiphyCode) { return alexander.reply(from, 'Gagal mengambil kode giphy', id) }
                const giphyCode = getGiphyCode[0].replace(/[-\/]/gi, '')
                const smallGifUrl = 'https://media.giphy.com/media/' + giphyCode + '/giphy-downsized.gif'
                alexander.sendGiphyAsSticker(from, smallGifUrl).then(() => {
                    alexander.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                ...
                alexander.sendGiphyAsSticker(from, smallGifUrl)
                .then(() => {
                    alexander.reply(from, 'Here\'s your sticker')
                    console.log(`Sticker Processed for ${processTime(t, moment())} Second`)
                })
                .catch(() => {
                    alexander.reply(from, `Ada yang error!`, id)
                })
            } else {
                await alexander.reply(from, 'Maaf, command sticker giphy hanya bisa menggunakan link dari giphy.  [Giphy Only]', id)
            }
            break
      case 'qrread':
      ...
      break			
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                alexander.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, 'Ini makasih!',id)
                    })
                    .catch(() => {
                        alexander.reply(from, 'Ada yang error!')
                    })
            } else {
                await alexander.reply(from, `Tidak ada gambar! Silahkan kirim gambar dengan caption ${prefix}meme <teks_atas> | <teks_bawah>\ncontoh: ${prefix}meme teks atas | teks bawah`, id)
            }
            break
        case 'quotemaker':
        ...
        case 'nulis':
            ...
            await alexander.sendImage(from, `${nulisp}`, '', 'Nih...', id)
            .catch(() => {
        ...
        case 'listsurah':
            try {
                ...
                .then((response) => {
                    ...
                    alexander.reply(from, hehex, id)
                })
            } catch(err) {
                alexander.reply(from, err, id)
            }
            break
        case 'infosurah':
            if (args.length == 0) return alexander.reply(from, `*_${prefix}infosurah <nama surah>_*\nMenampilkan informasi lengkap mengenai surah tertentu. Contoh penggunan: ${prefix}infosurah al-baqarah`, message.id)
                ...
                alexander.reply(from, pesan, message.id)
              break
        case 'surah':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}surah <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}surah al-baqarah 1\n\n*_${prefix}surah <nama surah> <ayat> en/id_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahannya dalam bahasa Inggris / Indonesia. Contoh penggunaan : ${prefix}surah al-baqarah 1 id`, message.id)
              ...
              break
        case 'tafsir':
            if (args.length == 0) return alexander.reply(from, `*_${prefix}tafsir <nama surah> <ayat>_*\nMenampilkan ayat Al-Quran tertentu beserta terjemahan dan tafsirnya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}tafsir al-baqarah 1`, message.id)
              ...
              bhs = ""
                ...
                if(!isNaN(nmr)) {
                  ...
                  if(isNaN(ayat)) {
                    ...
                    pesan = pesan + "\n\n(Q.S. "+data.surah.name.transliteration.id+":"+args[1]+")"
                    await alexander.sendFileFromUrl(from, data.audio.secondary[0])
                    await aruga.reply(from, pesan, message.id)
                  }
              }
              break
        case 'jsolat':
            if (args.length == 0) return alexander.reply(from, `Untuk melihat jadwal solat dari setiap daerah yang ada\nketik: ${prefix}jsolat [daerah]\n\nuntuk list daerah yang ada\nketik: ${prefix}daerah`, id)
            const solatx = body.slice(8)
            const solatj = await rugaapi.jadwaldaerah(solatx)
            await aruga.reply(from, solatj, id)
            .catch(() => {
                alexander.reply(from, 'Pastikan daerah kamu ada di list ya!', id)
            })
            break
        case 'daerah':
        ...
        //Kristiani
            case 'alkitab':
            if (args.length == 0) return alexander.reply(from, `Kirim perintah *${prefix}alkitab* [ Ayat ]\n\n*Contoh :* ${prefix}alkitab matius`, id)
            ...
            try {
                ...
                await alexander.reply(from, alkitabb, id)
            } catch (err){
                console.log(err)
            }
            break
	...
	//Group All User
	case 'grouplink':
            if (!isBotGroupAdmins) return alexander.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            if (isGroupMsg) {
                const inviteLink = await aruga.getGroupInviteLink(groupId);
                alexander.sendLinkWithAutoPreview(from, inviteLink, `\nLink group *${name}* Gunakan *${prefix}revoke* untuk mereset Link group`)
            } else {
            	alexander.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            }
            break
	case "revoke":
	if (!isBotGroupAdmins) return alexander.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                alexander.reply(from, `Berhasil Revoke Grup Link gunakan *${prefix}grouplink* untuk mendapatkan group invite link yang terbaru`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        //Media
        case 'ytmp3':
            if (args.length == 0) return alexander.reply(from, `Untuk mendownload lagu dari youtube\nketik: ${prefix}ytmp3 [link_yt]`, id)
            const linkmp3 = args[0].replace('https://youtu.be/','').replace('https://www.youtube.com/watch?v=','')
			rugaapi.ytmp3(`https://youtu.be/${linkmp3}`)
            .then(async(res) => {
				if (res.error) return aruga.sendFileFromUrl(from, `${res.url}`, '', `${res.error}`)
				await alexander.sendFileFromUrl(from, `${res.result.thumb}`, '', `Lagu ditemukan\n\nJudul: ${res.result.title}\nDesc: ${res.result.desc}\nSabar lagi dikirim`, id)
				await alexander.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
        ...
        case 'ytmp4':
            ...
            .then(async(res) => {
				...
				await alexander.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
					alexander.reply(from, `URL Ini ${args[0]} Sudah pernah di Download sebelumnya. URL akan di Reset setelah 1 Jam/60 Menit`, id)
				})
			})
            break
		case 'fb':
		case 'facebook':
			if (args.length == 0) return alexander.reply(from, `Untuk mendownload video dari link facebook\nketik: ${prefix}fb [link_fb]`, id)
			rugaapi.fb(args[0])
			.then(async (res) => {
				const { link, linkhd, linksd } = res
				if (res.status == 'error') return alexander.sendFileFromUrl(from, link, '', 'Maaf url anda tidak dapat ditemukan', id)
				await alexander.sendFileFromUrl(from, linkhd, '', 'Nih ngab videonya', id)
				.catch(async () => {
					await alexander.sendFileFromUrl(from, linksd, '', 'Nih ngab videonya', id)
					.catch(() => {
						alexander.reply(from, 'Maaf url anda tidak dapat ditemukan', id)
					})
				})
			})
			break
       ...
       case 'moddroid':
            if (!isGroupMsg) return alexander.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 0) return alexander.reply(from, 'Kirim perintah *#moddroid [query]*\nContoh : *#moddroid pou*', id)
            try {
                ...
                alexander.sendFileFromUrl(from, modo.image, 'MODDROID.jpg', resmod, id)
            } catch (err) {
                console.log(err)
            }
            break
        case 'happymod':
            if (!isGroupMsg) return alexander.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (args.length == 0) return alexander.reply(from, 'Kirim perintah *#happymod [query]*\nContoh : *#happymod pou*', id)
            try {
                ...
                alexander.sendFileFromUrl(from, modo.image, 'HAPPYMOD.jpg', resmod, id)
            } catch (err) {
                console.log(err)
            }
            break
        ...
        case 'fml':
            ...
            .catch(() => {
                alexander.reply(from, 'Hayolohhh, ada yang error!!', id)
            })
            break
      	case 'motivasi':
            ...
            .then(body => {
                ...
                alexander.reply(from, randommotivasi, id)
            })
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
	      ...
	      case 'howgay':
            ...
            .then(body => {
                ...
                alexander.reply(from, randomhowgay, id)
            })
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
        ...
        case 'fakta':
            ...
            .then(body => {
                ...
                alexander.reply(from, randomnix, id)
            })
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
        ...
        case 'katabijak':
            ...
            .then(body => {
                ...
                alexander.reply(from, randombijak, id)
            })
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
        ...
        case 'quote':
            ...
            await alexander.reply(from, quotex, id)
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
            break
    		case 'cerpen':
	     	...
	     	case 'cersex':
			      rugaapi.cersex()
			      .then(async (res) => {
			    	await alexander.reply(from, res.result, id)
		      	})
		      	break
	    	case 'puisi':
		      	rugaapi.puisi()
        ...
        case 'memeindo':
            ...
            await alexander.sendFileFromUrl(from, memeindox, 'memeindo.jpeg', 'Nih.....', id)
            .catch(() => {
                alexander.reply(from, 'Hayolohhh, ada yang error!!', id)
            })
            break
        case 'darkjokes':
            ...
            await alexander.sendFileFromUrl(from, darkjokesx, 'memeindo.jpeg', 'Nih.....', id)
            .catch(() => {
                alexander.reply(from, 'Hayolohhh, ada yang error!!', id)
            })
            break
        case 'anime':
            if (args.length == 0) return alexander.reply(from, `Untuk menggunakan ${prefix}anime\nSilahkan ketik: ${prefix}anime [query]\nContoh: ${prefix}anime random\n\nquery yang tersedia:\nrandom, waifu, husbu, neko`, id)
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    alexander.sendFileFromUrl(from, randomnimex, '', 'Nee..', id)
                })
                .catch(() => {
                    alexander.reply(from, 'Ada yang Error!', id)
                })
            } else {
                alexander.reply(from, `Maaf query tidak tersedia. Silahkan ketik ${prefix}anime untuk melihat list query`)
            }
            break
        case 'kpop':
            if (args.length == 0) return alexander.reply(from, `Untuk menggunakan ${prefix}kpop\nSilahkan ketik: ${prefix}kpop [query]\nContoh: ${prefix}kpop bts\n\nquery yang tersedia:\nblackpink, exo, bts`, id)
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    alexander.sendFileFromUrl(from, randomkpopx, '', 'Nee..', id)
                })
                .catch(() => {
                    alexander.reply(from, 'Ada yang Error!', id)
                })
            } else {
                alexander.reply(from, `Maaf query tidak tersedia. Silahkan ketik ${prefix}kpop untuk melihat list query`)
            }
            break
        case 'memes':
	...
	case 'dewabatch':
		...
		.then(async(res) => {
		await alexander.sendFileFromUrl(from, `${res.link}`, '', `${res.text}`, id)
		})
		break
        case 'images':
            if (args.length == 0) return alexander.reply(from, `Untuk mencari gambar dari pinterest\nketik: ${prefix}images [search]\ncontoh: ${prefix}images naruto`, id)
            const cariwall = body.slice(8)
            const hasilwall = await images.fdci(cariwall)
            await alexander.sendFileFromUrl(from, hasilwall, '', '', id)
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'sreddit':
            if (args.length == 0) return alexander.reply(from, `Untuk mencari gambar dari sub reddit\nketik: ${prefix}sreddit [search]\ncontoh: ${prefix}sreddit naruto`, id)
	    ...
	    break
        case 'resep':
            if (args.length == 0) return alexander.reply(from, `Untuk mencari resep makanan\nCaranya ketik: ${prefix}resep [search]\n\ncontoh: ${prefix}resep tahu`, id)
            const cariresep = body.slice(7)
            const hasilresep = await resep.resep(cariresep)
            await alexander.reply(from, hasilresep + '\n\nIni kak resep makanannya..', id)
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'nekopoi':
            ...
            .then((result) => {
                ...
                .then((res) => {
                    ...
                    alexander.reply(from, `Title: ${res.title}\n\nLink:\n${heheq}\nmasih tester bntr :v`)
                })
            })
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            ...
            break
        case 'stalkig':
            if (args.length == 0) return alexander.reply(from, `Untuk men-stalk akun instagram seseorang\nketik ${prefix}stalkig [username]\ncontoh: ${prefix}stalkig ini.arga`, id)
        ...
        case 'wiki':
            ...
            break
        case 'cuaca':
            if (args.length == 0) return alexander.reply(from, `Untuk melihat cuaca pada suatu daerah\nketik: ${prefix}cuaca [daerah]`, id)
            const cuacaq = body.slice(7)
            const cuacap = await rugaapi.cuaca(cuacaq)
            await alexander.reply(from, cuacap, id)
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
            break
        case 'lyrics':
        case 'lirik':
            if (args.length == 0) return alexander.reply(from, `Untuk mencari lirik dari sebuah lagu\bketik: ${prefix}lirik [judul_lagu]`, id)
            rugaapi.lirik(body.slice(7))
            .then(async (res) => {
                await alexander.reply(from, `Lirik Lagu: ${body.slice(7)}\n\n${res}`, id)
            })
            break
        case 'chord':
            if (args.length == 0) return alexander.reply(from, `Untuk mencari lirik dan chord dari sebuah lagu\bketik: ${prefix}chord [judul_lagu]`, id)
        ...
        case 'ss': //jika error silahkan buka file di folder settings/api.json dan ubah apiSS 'API-KEY' yang kalian dapat dari website https://apiflash.com/
            if (args.length == 0) return alexander.reply(from, `Membuat bot men-screenshot sebuah web\n\nPemakaian: ${prefix}ss [url]\n\ncontoh: ${prefix}ss http://google.com`, id)
        ...
        case 'play'://silahkan kalian custom sendiri jika ada yang ingin diubah
            if (args.length == 0) return alexander.reply(from, `Untuk mencari lagu dari youtube\n\nPenggunaan: ${prefix}play judul lagu`, id)
            ...
            .then(async (res) => {
				...
				.then(async(res) => {
					if (res.status == 'error') return alexander.sendFileFromUrl(from, `${res.link}`, '', `${res.error}`)
					await alexander.sendFileFromUrl(from, `${res.thumb}`, '', `Lagu ditemukan\n\nJudul ${res.title}\n\nSabar lagi dikirim`, id)
					await alexander.sendFileFromUrl(from, `${res.link}`, '', '', id)
					.catch(() => {
						alexander.reply(from, `URL Ini ${args[0]} Sudah pernah di Download sebelumnya. URL akan di Reset setelah 1 Jam/60 Menit`, id)
					})
				})
            ...
            break
		case 'movie':
			if (args.length == 0) return alexander.reply(from, `Untuk mencari suatu movie dari website sdmovie.fun\nketik: ${prefix}movie judulnya`, id)
			rugaapi.movie((body.slice(7)))
			.then(async (res) => {
				if (res.status == 'error') return alexander.reply(from, res.hasil, id)
				await alexander.sendFileFromUrl(from, res.link, 'movie.jpg', res.hasil, id)
			})
			break
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                alexander.reply(from, 'Searching....', id)
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		alexander.reply(from, 'Maaf, saya tidak tau ini anime apa, pastikan gambar yang akan di Search tidak Buram/Kepotong', id)
                	}
        ...
        case 'resi':
            ...
            break
        case 'tts':
            if (args.length == 0) return alexander.reply(from, `Mengubah teks menjadi sound (google voice)\nketik: ${prefix}tts <kode_bahasa> <teks>\ncontoh : ${prefix}tts id halo\nuntuk kode bahasa cek disini : https://anotepad.com/note/read/5xqahdy8`)
            const ttsGB = require('node-gtts')(args[0])
            const dataText = body.slice(8)
                if (dataText === '') return alexander.reply(from, 'apa teksnya syg..', id)
                try {
                    ttsGB.save('./media/tts.mp3', dataText, function () {
                    alexander.sendPtt(from, './media/tts.mp3', id)
                    })
                } catch (err) {
                    alexander.reply(from, err, id)
                }
        ...
        case 'ceklokasi':
            ...
            if (zoneStatus.kode !== 200) alexander.sendText(from, 'Maaf, Terjadi error ketika memeriksa lokasi yang anda kirim.')
            let datax = ''
            ...
            alexander.sendText(from, text)
            break
        case 'shortlink':
            if (args.length == 0) return alexander.reply(from, `ketik ${prefix}shortlink <url>`, id)
            if (!isUrl(args[0])) return alexander.reply(from, 'Maaf, url yang kamu kirim tidak valid.', id)
            const shortlink = await urlShortener(args[0])
            await alexander.sendText(from, shortlink)
            .catch(() => {
                alexander.reply(from, 'Ada yang Error!', id)
            })
            break
		case 'bapakfont':
			if (args.length == 0) return alexander.reply(from, `Mengubah kalimat menjadi alayyyyy\n\nketik ${prefix}bapakfont kalimat`, id)
			rugaapi.bapakfont(body.slice(11))
			.then(async(res) => {
				await alexander.reply(from, `${res}`, id)
			})
			break
        case 'hilihfont':
            if (args.length == 0) return alexander.reply(from, `Mengubah kalimat menjadi hilih gitu deh\n\nketik ${prefix}hilihfont kalimat`, id)
		...
		//Fun Menu
    case 'tod':
    alexander.reply(from, 'Sebelum bermain berjanjilah akan melaksanakan apapun perintah yang diberikan.\n\nSilahkan Pilih:\n➥ #truth\n➥ #dare', id)
    break
    case 'truth':
    if (!isGroupMsg) return alexander.reply(from, menuId.textPrem())
    ...
    if (!isGroupMsg) return alexander.reply(from, menuId.textPrem())
		...
		case 'klasmen':
            ...
            await alexander.sendTextWithMentions(from, textKlas)
			break
	    ...
	    case 'add':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
	        if (args.length !== 1) return alexander.reply(from, `Untuk menggunakan ${prefix}add\nPenggunaan: ${prefix}add <nomor>\ncontoh: ${prefix}add 628xxx`, id)
                try {
                    await alexander.addParticipant(from,`${args[0]}@c.us`)
                } catch {
                    alexander.reply(from, 'Tidak dapat menambahkan target', id)
                }
            break
        case 'kick':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length === 0) return alexander.reply(from, 'Maaf, format pesan salah.\nSilahkan tag satu atau lebih orang yang akan dikeluarkan', id)
            if (mentionedJidList[0] === botNumber) return await alexander.reply(from, 'Maaf, format pesan salah.\nTidak dapat mengeluarkan akun bot sendiri', id)
            await alexander.sendTextWithMentions(from, `Request diterima, mengeluarkan:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await alexander.sendText(from, 'Gagal, kamu tidak bisa mengeluarkan admin grup.')
                await alexander.removeParticipant(groupId, mentionedJidList[i])
            }
            break
        case 'promote':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return alexander.reply(from, 'Maaf, hanya bisa mempromote 1 user', id)
            if (groupAdmins.includes(mentionedJidList[0])) return await alexander.reply(from, 'Maaf, user tersebut sudah menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await alexander.reply(from, 'Maaf, format pesan salah.\nTidak dapat mempromote akun bot sendiri', id)
            await alexander.promoteParticipant(groupId, mentionedJidList[0])
            await alexander.sendTextWithMentions(from, `Request diterima, menambahkan @${mentionedJidList[0].replace('@c.us', '')} sebagai admin.`)
            break
        case 'demote':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            if (mentionedJidList.length !== 1) return alexander.reply(from, 'Maaf, hanya bisa mendemote 1 user', id)
            if (!groupAdmins.includes(mentionedJidList[0])) return await alexander.reply(from, 'Maaf, user tersebut belum menjadi admin.', id)
            if (mentionedJidList[0] === botNumber) return await alexander.reply(from, 'Maaf, format pesan salah.\nTidak dapat mendemote akun bot sendiri', id)
            await alexander.demoteParticipant(groupId, mentionedJidList[0])
            await alexander.sendTextWithMentions(from, `Request diterima, menghapus jabatan @${mentionedJidList[0].replace('@c.us', '')}.`)
            break
        case 'bye':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            alexander.sendText(from, 'Good bye... ( ⇀‸↼‶ )').then(() => alexander.leaveGroup(groupId))
            break
        case 'del':
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!quotedMsg) return alexander.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
            if (!quotedMsgObj.fromMe) return alexander.reply(from, `Maaf, format pesan salah silahkan.\nReply pesan bot dengan caption ${prefix}del`, id)
            alexander.deleteMessage(quotedMsgObj.chatId, quotedMsgObj.id, false)
            break
        case 'tagall':
        case 'everyone':
            if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
		...
		case 'simisimi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			alexander.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			break
		case 'simi':
			if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
			if (args.length !== 1) return alexander.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			if (args[0] == 'on') {
				simi.push(chatId)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
                alexander.reply(from, 'Mengaktifkan bot simi-simi!', id)
			} else if (args[0] == 'off') {
				let inxx = simi.indexOf(chatId)
				simi.splice(inxx, 1)
				fs.writeFileSync('./settings/simi.json', JSON.stringify(simi))
				alexander.reply(from, 'Menonaktifkan bot simi-simi!', id)
			} else {
				alexander.reply(from, `Untuk mengaktifkan simi-simi pada Group Chat\n\nPenggunaan\n${prefix}simi on --mengaktifkan\n${prefix}simi off --nonaktifkan\n`, id)
			}
			break
		case 'katakasar':
			if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			alexander.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\nApasih kegunaan Fitur Ini? Apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			break
		case 'kasar':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
			if (args.length !== 1) return alexander.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\nApasih kegunaan Fitur Ini? Apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			if (args[0] == 'on') {
				ngegas.push(chatId)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				alexander.reply(from, 'Fitur Anti Kasar sudah di Aktifkan', id)
			} else if (args[0] == 'off') {
				let nixx = ngegas.indexOf(chatId)
				ngegas.splice(nixx, 1)
				fs.writeFileSync('./settings/ngegas.json', JSON.stringify(ngegas))
				alexander.reply(from, 'Fitur Anti Kasar sudah di non-Aktifkan', id)
			} else {
				alexander.reply(from, `Untuk mengaktifkan Fitur Kata Kasar pada Group Chat\n\nApasih kegunaan Fitur Ini? Apabila seseorang mengucapkan kata kasar akan mendapatkan denda\n\nPenggunaan\n${prefix}kasar on --mengaktifkan\n${prefix}kasar off --nonaktifkan\n\n${prefix}reset --reset jumlah denda`, id)
			}
			break
		case 'reset':
            ...
            if(reset){
				await alexander.sendText(from, "Klasemen telah direset.")
            }
			break
		case 'mutegrup':
			if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
			if (args.length !== 1) return alexander.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
            if (args[0] == 'on') {
				alexander.setGroupToAdminsOnly(groupId, true).then(() => aruga.sendText(from, 'Berhasil mengubah agar hanya admin yang dapat chat!'))
			} else if (args[0] == 'off') {
				alexander.setGroupToAdminsOnly(groupId, false).then(() => aruga.sendText(from, 'Berhasil mengubah agar semua anggota dapat chat!'))
			} else {
				alexander.reply(from, `Untuk mengubah settingan group chat agar hanya admin saja yang bisa chat\n\nPenggunaan:\n${prefix}mutegrup on --aktifkan\n${prefix}mutegrup off --nonaktifkan`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			...
			if (isMedia && type == 'image' || isQuotedImage) {
				...
				const imageBase64 = `data:${_mimetype};base64,${mediaData.toString('base64')}`
				await alexander.setGroupIcon(groupId, imageBase64)
			} else if (args.length === 1) {
				if (!isUrl(url)) { await aruga.reply(from, 'Maaf, link yang kamu kirim tidak valid.', id) }
				alexander.setGroupIconByUrl(groupId, url).then((r) => (!r && r !== undefined)
				? alexander.reply(from, 'Maaf, link yang kamu kirim tidak memuat gambar.', id)
				: alexander.reply(from, 'Berhasil mengubah profile group', id))
			} else {
				alexander.reply(from, `Commands ini digunakan untuk mengganti icon/profile group chat\n\n\nPenggunaan:\n1. Silahkan kirim/reply sebuah gambar dengan caption ${prefix}setprofile\n\n2. Silahkan ketik ${prefix}setprofile linkImage`)
			}
			break
		case 'welcome':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return alexander.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
            if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
			if (args.length !== 1) return alexander.reply(from, `Membuat BOT menyapa member yang baru join kedalam group chat!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`, id)
			if (args[0] == 'on') {
				welcome.push(chatId)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				alexander.reply(from, 'Welcome Message sekarang diaktifkan!', id)
			} else if (args[0] == 'off') {
				let xporn = welcome.indexOf(chatId)
				welcome.splice(xporn, 1)
				fs.writeFileSync('./settings/welcome.json', JSON.stringify(welcome))
				alexander.reply(from, 'Welcome Message sekarang dinonaktifkan', id)
			} else {
				alexander.reply(from, `Membuat BOT menyapa member yang baru join kedalam group chat!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`, id)
			}
			break
        ...
        if (!isOwner) return alexander.reply(from, 'Maaf, perintah ini hanya dapat dipakai oleh owner grup!', id)
        if (!isBotGroupAdmins) return alexander.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            const allMem = await alexander.getGroupMembers(groupId)
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {

                } else {
                    await alexander.removeParticipant(groupId, allMem[i].id)
                }
            }
            alexander.reply(from, 'Success kick all member', id)
        break

        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return alexander.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            ...
            if (args[0] == 'add') {
                ...
                alexander.reply(from, 'Success banned target!')
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
                banned.splice(xnxx,1)
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                alexander.reply(from, 'Success unbanned target!')
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                banned.push(mentionedJidList[i])
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                alexander.reply(from, 'Success ban target!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            if (!isOwnerBot) return alexander.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return alexander.reply(from, `Untuk broadcast ke semua chat ketik:\n${prefix}bc [isi chat]`)
            let msg = body.slice(4)
            const chatz = await alexander.getAllChatIds()
            for (let idk of chatz) {
                var cvk = await alexander.getChatById(idk)
                if (!cvk.isReadOnly) alexander.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) alexander.sendText(idk, `〘 *A R U G A  B C* 〙\n\n${msg}`)
            }
            alexander.reply(from, 'Broadcast Success!', id)
            break
        case 'leaveall': //mengeluarkan bot dari semua group serta menghapus chatnya
            if (!isOwnerBot) return alexander.reply(from, 'Perintah ini hanya untuk Owner bot', id)
            const allChatz = await alexander.getAllChatIds()
            const allGroupz = await alexander.getAllGroups()
            for (let gclist of allGroupz) {
                await aruga.sendText(gclist.contact.id, `Maaf bot sedang pembersihan, total chat aktif : ${allChatz.length}`)
                await alexander.leaveGroup(gclist.contact.id)
                await aruga.deleteChat(gclist.contact.id)
            }
            alexander.reply(from, 'Success leave all group!', id)
            break
        case 'clearall': //menghapus seluruh pesan diakun bot
            if (!isOwnerBot) return alexander.reply(from, 'Perintah ini hanya untuk Owner bot', id)
            const allChatx = await alexander.getAllChats()
            for (let dchat of allChatx) {
                await alexander.deleteChat(dchat.id)
            }
            alexander.reply(from, 'Success clear all chat!', id)
            break
        default:
            break
        }
		...
		if ((!isCmd && isGroupMsg && isSimi) && message.type === 'chat') {
			...
			.then((res) => {
				if (res.data.status == 403) return alexander.sendText(ownerNumber, `${res.data.result}\n\n${res.data.pesan}`)
				alexander.reply(from, `Simi berkata: ${res.data.result}`, id)
			})
			.catch((err) => {
				alexander.reply(from, `${err}`, id)
			})
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        ...
                        if(denda){
                            await alexander.reply(from, "Jangan badword bodoh\nDenda +5.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
